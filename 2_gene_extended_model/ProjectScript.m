...
%ChunkSize     = 5;
PoolSize      = 200e3;  % this used to be 200e3, 187e3 +/- 56e3 (von der Haar, 2008)
...
K_gen         = 5000;  % generations (number of accepted mutations) we used to just run 2e4 gens, now we just do 1000 accepts
Fit           = .01;
count_update  = 0;
count_fail    = 0;
...
for i=1:1 %Total Experiments
    ...
    [ramp_2,bod_2] = first_codon_fraction(Genes(2),{'TAT','TAC'});
		...
		fprintf(fid, '%i,', Density0{1}(:,end));
		fprintf(fid,'\n');
		fclose(fid);
		fid = fopen(strcat(csv_folder,'2_gene_density.dat'), 'w');
		fprintf(fid, '%i,', Density0{2}(:,end));
		fprintf(fid,'\n');
		fclose(fid);
		...
		while (k < K_gen) 
				...
				if (accept == 1)
					...
					%Recalculate the ratio of fast to slow codons
						...
						fprintf(fid, '%i,', Density{1}(:,end));
						fprintf(fid,'\n');
						fclose(fid);
						fid = fopen(strcat(csv_folder,'2_gene_density.dat'), 'a');
						fprintf(fid, '%i,', Density{2}(:,end));
						fprintf(fid,'\n');
						fclose(fid);
						...
						csvwrite(strcat(csv_folder,'current_gen.dat'),gen);
